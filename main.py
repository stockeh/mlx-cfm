...
from datasets import sample_moons, sample_8gaussians

parser = argparse.ArgumentParser(add_help=True)
parser.add_argument('-m', '--method', type=str, default='vp',
                    choices=['cfm', 'vp'], help='cfm method')
parser.add_argument('--sigma', type=float, default=0.1, help='sigma for cfm')
parser.add_argument('--solver', type=str, default='euler',
                    choices=['dopri5', 'euler'], help='ODE solver')
parser.add_argument('-b', '--batch_size', type=int,
                    default=512, help='batch size')
parser.add_argument('-e', '--steps', type=int,
                    default=20000, help='number of steps')
...
class NeuralNetwork:
    ...
    def eval_fn(self, X, T):
        t, xt, ut = self.flow_matcher.\
            sample_location_and_conditional_flow(X, T)
...
def main(args):
    ...
    net = NeuralNetwork(model, optimizer, flow_matcher, integrator,
                        sample_8gaussians, sample_moons)
    ...
    ax.scatter(traj[0, :, 0], traj[0, :, 1], s=10,
               alpha=0.8, c='k', label=r'$\mathbf{x}_0 \sim \pi(\mathbf{x})$')
    ax.scatter(traj[:, :, 0], traj[:, :, 1], s=0.2,
               alpha=0.15, c='k')
    ax.scatter(traj[-1, :, 0], traj[-1, :, 1], s=4,
               alpha=1, c='blue', label=r'$\mathbf{x}_1 \approx \mathbf{x}^\prime\;[\sim p(\mathbf{x})]$')
    ...
    def plot_kde(data, x_grid, y_grid, ax, bandwidth=0.3, cmap='Blues', alpha=0.5):
        ...
        ax.contourf(x_grid, y_grid, density,
                    levels=100, cmap=cmap, alpha=alpha)
    ...
    y_min = traj[:, :, 1].min() - 1
    y_max = traj[:, :, 1].max() + 1
    x_grid, y_grid = mx.meshgrid(mx.linspace(
        x_min, x_max, 100), mx.linspace(y_min, y_max, 100))

    plot_kde(prior_data, x_grid, y_grid, ax,
             bandwidth=0.5, cmap='Reds', alpha=0.2)
    plot_kde(last_data, x_grid, y_grid, ax,
             bandwidth=0.5, cmap='Purples', alpha=0.2)
    ax.set_xticks([])
    ax.set_yticks([])
